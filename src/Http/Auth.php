...
namespace Samueletur\GalaxPay\Http;

use Samueletur\GalaxPay\Http\Config;
...
final class Auth
{
     ...
     * Request
     * 
     * @var \Samueletur\GalaxPay\Http\Request
     */
    private $request;
     ...
     * Active session
     * 
     * @var \Samueletur\GalaxPay\Contracts\Session
     */
    private $session;

    /**
     * @param \Samueletur\GalaxPay\Http\Config $config
     */
    public function __construct(Config $config)
    {
        ...
        if ($this->config->get('session_driver') == 'database') {
            $this->session = new \Samueletur\GalaxPay\Sessions\Database($this->config);
        } else {
            $this->clientGalaxId = $this->config->get('credentials.client.id');
            $this->session = new \Samueletur\GalaxPay\Sessions\File($this->config);
        }
     ...
     * 
     * @return void
     * @throws \Samueletur\GalaxPay\Exceptions\AuthorizationException
     * @throws \Samueletur\GalaxPay\Exceptions\GalaxPayRequestException
     */
